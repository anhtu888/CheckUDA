(function($) {
    ...
    $(document).ready(function() {
        ...
        $('.scroll').on('click', function() { $("html, body").animate({ scrollTop: $('#reach-to').offset().top }, 2000); return false; });
        ...
        $('.menu-icon a').on('click', function() { $('body').toggleClass('hidden-body'); });
        $('.icon-find').on('click', function() { $('.location-footer-map').addClass('location-open'); return false; });
        $('.icon-find-location').on('click', function() { $('.location-footer-map').removeClass('location-open'); return false; });
        $('.date-pick').datepicker()
    });
    $(window).on('load', function() {
        initComponents();
        $('.menu-main ul li').each(function() { if ($(this).find('.drop-nav').length) { $(this).append('<span class="drop-nav-arrow"><i class="fa fa-angle-down"></i></span>') } });
        app.dropdown();
    ...
    $('.top-arrow').on('click', function() { $("html, body").animate({ scrollTop: 0 }, 600); return false; });
    $(window).on('scroll', function() { headerFix(); if ($(this).scrollTop() > 500) { $('.top-arrow').fadeIn(); } else { $('.top-arrow').fadeOut(); } return false; });

    function initComponents() { initBg(); }
...
$(document).on('ready', function() {
    ...
    // if (document.getElementById('footer-map') != null) { google.maps.event.addDomListener(window, 'load', footer_map_initialize); }
    ...
    $("form[name='contact-form']").on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $(".alert .close").on('click', function() { $(".alert-container").hide(); });
            }
        });
        return false;
    });
...
function headerFix() {
    ...
    if (topPos > header_height) { if ((window_height < document_height) && $('.header-part').hasClass('sticky')) { $('.header-part').addClass('sticky-fixed'); } } else { $('.header-part').removeClass('sticky-fixed'); }
}
