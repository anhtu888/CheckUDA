...
import TableAdminUser from './TableAdminUser';
export default class Index extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            listAdminUser: [], // Khởi tạo mảng để lưu trữ danh sách element
    ...
    componentWillMount() {
        if (localStorage && localStorage.getItem('listAdminUser')) {
            var listAdminUser = JSON.parse(localStorage.getItem('listAdminUser'));
            this.setState({
                listAdminUser: listAdminUser
            });
        }
    }
    //Mở form Insert
    ...
    onSubmit = (data) => {
        var { listAdminUser } = this.state;
        if (data.id === '') {
            data.id = this.randomId();
            listAdminUser.push(data);
        } else {
            var index = this.findIndex(data.id);
            listAdminUser[index] = data;
        }
        this.setState({
            listAdminUser: listAdminUser,
            isHidden: false
        ...
        localStorage.setItem('listAdminUser', JSON.stringify(listAdminUser));
    }
    //Chức năng Delete
    onDelete = (id) => {
        var { listAdminUser } = this.state;
    ...
    onUpdate = (id) => {
        var { listAdminUser } = this.state;
        var index = this.findIndex(id);
        var elementEdit = listAdminUser[index];
        if (index != -1) {
            this.setState({
    ...
    findIndex = (id) => {
        var { listAdminUser } = this.state;
        var result = -1;
        listAdminUser.forEach((item, index) => {
            if (item.id === id) {
                result = index;
            }
        });
    ...
    render() {
        var { listAdminUser, isHidden, elementEdit, fiter } = this.state;

        var hiddenInsertAdminUser = isHidden ? <InsertAdminUser
        ...
        if (fiter) {
            if (fiter.name) {
                listAdminUser = listAdminUser.filter((item)=>{
                ...
                <div className="col-xs-12" style={{ width: '100%' }}>
                    <div className="card">
                        <div className="card-header">
                            <h3 className="card-title text-primary " style={{ fontSize: '30px' }}><i className="fas fa-align-justify" /> <b>Tài khoảng admin</b></h3>
                        </div>
